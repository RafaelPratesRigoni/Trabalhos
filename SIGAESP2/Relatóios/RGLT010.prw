...
BeginSql alias "QRYSA2"     
	...
	LEFT JOIN %TABLE:ZLS% (NOLOCK) ZLS
			ON(	ZLS.%NOTDEL%
				AND A2_L_TANQ = ZLS_COD)
	WHERE     SA2.A2_FILIAL = %XFILIAL:SA2%
			AND SA2.%NOTDEL%
			AND A2_L_TPFOR = 'P'
			AND SA2.A2_COD BETWEEN %EXP:MV_PAR01% AND %EXP:MV_PAR02%
			AND SA2.A2_LOJA BETWEEN %EXP:MV_PAR03% AND %EXP:MV_PAR04%
			AND SA2.A2_L_TANQ BETWEEN %EXP:MV_PAR05% AND %EXP:MV_PAR06%
	ORDER BY  SA2.A2_L_TANQ,
			SA2.A2_NREDUZ
