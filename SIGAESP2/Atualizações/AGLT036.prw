...
Static Function ProcBrw(cOpcBrw)
	...
	If nOpcao != 0
		aSX3 := FwSX3Util():GetAllFields(cAliasItm)

		For nI := 1 To Len(aSX3)
			If X3Uso(GetSX3Cache(aSX3[nI], "X3_USADO")) .AND. cNivel >= GetSX3Cache(aSX3[nI], "X3_NIVEL") .AND. aSX3[nI] != cCmpCod2
				nUsado++
		...
		If Len(aCols) > 0
			...
			Endif
		Else
			MsgAlert("Nao existem itens para este documento!", "Aten็ใo!")
			Return()
		Endif
...
Static Function LinOK()
	...
	If ( !aCols[n][Len(aCols[n])])
		...
		If Empty(cParticip)
			MsgAlert("O codigo do agregado deve ser informado!", "Aten็ใo!")
			Return(.F.)
		EndIf
	EndIf
...
Static Function TudoOk()
	...
	If ( nTam == 1  )
		...
		If ( aCols[nTam][Len(aCols[nTam])])
			MsgAlert("O item esta deletado! Informe o agregado!", "Aten็ใo!")
			Return(.F.)
		EndIf
		...
		If	(  Empty(aCols[nTam][nPosPart]) ) .or. (  Empty(aCols[nTam][nPosLjPa]) )
			MsgAlert("O agregado deve ser informado!", "Aten็ใo!")
			Return(.F.)
		EndIf
	EndIf
	...
	If nCntTit > 1
		MsgAlert("Existe mais de um TITULAR no mesmo Tanque. Altere para existir um ou nenhum TITULAR.", "Aten็ใo!")
		Return(.F.)
	EndIf

	//Existem dois Leiteiros
	If nCntLei > 1
		MsgAlert("Existe mais de um LEITEIRO no mesmo Tanque. Altere para existir um ou nenhum LEITEIRO.", "Aten็ใo!")
		Return(.F.)
	EndIf
