		...
		If Alltrim(FunName()) == "AGLT010"
			...
			If cVar == "ZLL_RETIRO"
				If Left(M->ZLL_RETIRO,1) == "T" //Val(soNumeros(M->ZLL_RETIRO)) > 10000
					M->ZLL_RETIRO := "E" + StrZero(Val(soNumeros(M->ZLL_RETIRO)),TamSX3("ZLL_RETIRO")[1]-1)//Atualiza o codigo
				ElseIF Val(soNumeros(M->ZLL_RETIRO)) > 0
					M->ZLL_RETIRO := "A" + StrZero(Val(soNumeros(M->ZLL_RETIRO)),TamSX3("ZLL_RETIRO")[1]-1)//Atualiza o codigo
				EndIf
				dbSeek(xFILIAL("SA1")+M->ZLL_RETIRO,.T.)
				If M->ZLL_RETIRO == SA1->A1_COD
					//Completa com P0000 o numero digitado no GetDados
					GdFieldPut("ZLL_RETILJ",SA1->A1_LOJA,n)//Atualiza a Loja
					GdFieldPut("ZLL_DCRRET",SA1->A1_NOME,n)//Atualiza a Descricao
				Else
					lRet := .F.
					xMagHelpFis("Mensagem","Aten็ใo, c๓digo de Produtor inexistente.","Insira um codigo de produtor existente!")
				EndIf
			Else
				If dbSeek(xFILIAL("SA1")+GdFieldGet("ZLL_RETIRO",n)+M->ZLL_RETILJ)
					GdFieldPut("ZLL_DCRRET",SA1->A1_NOME,n)//Atualiza a Descricao
				Else
					lRet := .F.
				...
				EndIf
			EndIf
		ElseIf Alltrim(FunName()) == "AGLT110"

			Private oModel 	:= FWModelActive()
			Private oStruZLL	:= oModel:GetModel('ZLLDETAIL')
			Private _cCodigo	:= oStruZLL:GetValue("ZLL_RETIRO")
			Private _cLoja	:= oStruZLL:GetValue("ZLL_RETILJ")
			...
			If cVar == "ZLL_RETIRO"
				IF Val(soNumeros(_cCodigo)) > 0 .and. Left(_cCodigo,1) <> "E"
					_cCodigo := "S" + StrZero(Val(soNumeros(_cCodigo)),TamSX3("ZLL_RETIRO")[1]-1)//Atualiza o codigo
					oStruZLL:LoadValue("ZLL_RETIRO",_cCodigo)
				EndIf
				dbSeek(xFILIAL("SA1")+_cCodigo,.T.)
				If _cCodigo == SA1->A1_COD
					//Completa com P0000 o numero digitado no GetDados
					oStruZLL:LoadValue("ZLL_RETILJ",SA1->A1_LOJA) //Atualiza a Loja
					oStruZLL:LoadValue("ZLL_DCRRET",SubStr(SA1->A1_NOME,1,TamSX3("ZLL_DCRRET")[1])) //Atualiza a Descricao]
				Else
					lRet := .F.
					xMagHelpFis("Mensagem","Aten็ใo, c๓digo de Produtor inexistente.","Insira um codigo de produtor existente!")
				EndIf
	...
	ElseIf cVar $ "ZLM_SA2COD/ZLM_SA2LJ"
		...
		If cVar == "ZLM_SA2COD"
			...
			ElseIF Val(soNumeros(M->ZLM_SA2COD)) > 0
				M->ZLM_SA2COD := "S" + StrZero(Val(soNumeros(M->ZLM_SA2COD)),TamSX3("ZLM_SA2COD")[1]-1)//Atualiza o codigo
			EndIf
			dbSeek(xFILIAL("SA1")+M->ZLM_SA2COD,.T.)
			If M->ZLM_SA2COD == SA1->A1_COD
				//Completa com P0000 o numero digitado no GetDados
				M->ZLM_SA2LJ  := SA1->A1_LOJA//Atualiza a Loja
				M->ZLM_SA2NOM := SubStr(SA1->A1_NOME,1,TamSX3("ZLM_SA2NOM")[1])//Atualiza a Descricao
			Else
				lRet := .F.
				xMagHelpFis("Mensagem","Aten็ใo, c๓digo de Produtor inexistente.","Insira um codigo de retiro existente!")
			EndIf
...
User Function ChkZLD(cCampo,cExp)
	...
	cQuery := " SELECT COUNT(ZLD_TICKET) AS CONT "
...
User Function AtuSeq(cAlias,nIndice,cSeek,nChv,cCampo)
	...
	aIndex	:= FwSIXUtil():GetAliasIndexes(cAlias)[nIndice]

	For nI := 1 To Len(aIndex)
		If AllTrim(aIndex[nI]) <> AllTrim(cCampo)
			cCmps += aIndex[nI]
		EndIf
	Next nI
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
User Function MostraF3(cAlias,cCpoCod,cCpoDesc,cDescCad,cNomeLabel,cCpoFil,cFiltro)
	...
	While (cAlias)->(!Eof())
		If !Empty(&cCpoFil) //Se precisa validar a Filial
			...
			EndIf
		ElseIf !Empty(&cFiltro) //Filtro gen้rico incluido na coluna   ZL9_TIPO == 'Q'
			If (cAlias)->&cFiltro
				aAdd(aDados,{ALLTRIM((cAlias)->&cCpoCod),ALLTRIM((cAlias)->&cCpoDesc),.F.})
			EndIf
...
User Function VolLeite(_fll,dt1,dt2,_nsetor,_LR,_for,_loj,_LIRT)
	...
	cQry2  := " SELECT SUM(ZLD_QTDBOM) TBOM, 0 TACI FROM " + RETSQLNAME("ZLD") + " ZLD "
	...
	TcQuery ChangeQuery(cQry2) New Alias "TRAB1"
	If !TRAB1->(Eof())
...
ฑฑฬออออออออออุอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUsuario   ณ															                             						บฑฑ
ฑฑฬออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบSetor     ณ Gestao do Leite	                                                                        					บฑฑ
ฑฑฬออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
User Function getCodN(cpCampo)
	...
	TcQuery ChangeQuery(cQuery) New Alias 'TCO'
...
User Function VolLteGrp()
	...
	Local _cGrupo := SA2->A2_L_GRUPO
	Local _cPeriodo := "% '" + DTOS(ZLE->ZLE_DTINI) + "' AND '" + DTOS(ZLE->ZLE_DTFIM) + "' %"
	Local _TOT     := 0
	...
	BeginSql alias _cAlias
		...
		FROM %Table:ZLD% ZLD
			...
			AND A2_L_GRUPO = %Exp:_cGrupo% AND A2.%NotDel%
		WHERE ZLD_FILIAL = %xFilial:ZLD%
		AND ZLD_SETOR  = %Exp:ZL2->ZL2_COD%
		AND ZLD_DTCOLE BETWEEN  %Exp:_cPeriodo%
		AND ZLD.%NotDel%
	EndSql
...
===============================================================================================================================
Descricao---------: No evento: Retorna(l๓gico) se o produtor fez parte dos maiores do m๊s de acordo com parโmetro informado.
						xnQtd = quantidade de produtores avalidados
					No relat๓rio: Retorna um array com a listagem dos maiores produtores
						{c๓digo, tipo(produtor ou grupo), volume}
===============================================================================================================================
Uso---------------: Programa principal
===============================================================================================================================
*/
User Function MAIORPROD(xnQtd,xcRet,xdDtIni,xdDtFim)
	...
	Local _cPeriodo := ""
	Local _nCont := 1
	DEFAULT xcRet := "E" //Padrใo de retorno para o evento.

	If xcRet == "E"
		_cPeriodo := "% '" + DTOS(ZLE->ZLE_DTINI) + "' AND '" + DTOS(ZLE->ZLE_DTFIM) + "' %"
	Else
		_cPeriodo := "% '" + DTOS(xdDtIni) + "' AND '" + DTOS(xdDtFim) + "' %"
	Endif

	//SELECT TOP %Exp:xnQtd% CODIGO, TIPO, SUM(ZLD_QTDBOM) QTD FROM (    ######## NAี FUNCIONA NO ORACLE
	BeginSql alias _cAlias
		SELECT  CODIGO, TIPO, SUM(QTD) QTD FROM (

		SELECT ZLD_RETIRO AS CODIGO, 'PRODUTOR' AS TIPO, SUM(ZLD_QTDBOM) QTD
		FROM %Table:ZLD% ZLD
			JOIN %Table:SA2% SA2 ON ZLD.ZLD_RETIRO = SA2.A2_COD AND ZLD.ZLD_RETILJ = SA2.A2_LOJA
					AND A2_L_MAIOR = 'S' AND A2_L_GRUPO = ' ' AND SA2.%NotDel%
		WHERE
		ZLD_DTCOLE BETWEEN  %Exp:_cPeriodo%
		AND ZLD.%NotDel%
		GROUP BY ZLD_RETIRO

		UNION ALL

		SELECT  A2_L_GRUPO AS CODIGO, 'GRUPO' AS TIPO, SUM(ZLD_QTDBOM) QTD
		FROM %Table:ZLD% ZLD
			JOIN %Table:SA2% SA2 ON ZLD.ZLD_RETIRO = SA2.A2_COD AND ZLD.ZLD_RETILJ = SA2.A2_LOJA
					AND A2_L_MAIOR = 'S' AND A2_L_GRUPO <> ' ' AND SA2.%NotDel%
		WHERE
		...
		ORDER BY 3 DESC
	EndSql
	//	WHERE ROWNUM <= %Exp:xnQtd% com este comando buga o resultado, achei mais fแcil tratar no advpl

	While !(_cAlias)->(Eof()) .and. _nCont <= xnQtd

		If xcRet == "E"
			If Alltrim((_cAlias)->TIPO) == "PRODUTOR" .AND. SA2->A2_COD == (_cAlias)->CODIGO
				_lRet := .t.
				Exit
			ElseIf Alltrim((_cAlias)->TIPO) == "GRUPO" .AND. SA2->A2_L_GRUPO == (_cAlias)->CODIGO
				_lRet := .t.
				Exit
			EndIf

		Else
			AADD(_aRet,{(_cAlias)->CODIGO,SubStr((_cAlias)->TIPO,1,1),(_cAlias)->QTD})
		EndIf
		(_cAlias)->(DbSkip())
		_nCont++
	EndDo
...
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบParametrosณ                                                                                      						บฑฑ
ฑฑฬออออออออออุอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบRetorno   ณ Nenhum					                                                                                  	บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUsuario   ณ															                             						บฑฑ
ฑฑฬออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
User Function getNewVersao(cMix)
	...
	If TVER->(!Eof())
		_cVersao := TVER->VERSAO
	EndIf
	TVER->(dbCloseArea())

	nVer := Val(_cVersao)
	nVer++
	_cVersao := ALLTRIM(str(nVer))
...
User Function GetCredPr(cMix,cProdutor,cLoja,cSetor,cLinha,cpVersao,lEvt)
	...
	DEFAULT lEvt     := .F.
	DEFAULT cpVersao := " "
	DEFAULT lAdici := .T. //Se considera o evento adicional de mercado na consulta
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
User Function getIndice(cpFilial,cpSetor,cpProd,cpLj,cpTpFaixa,cpMedia,dpData,cTPRet,xnQtdana)
	...
	DbSelectArea("ZL9")
	If Len(cpTpFaixa) == 3
		dbSetOrder(3)//ZL9_FILIAL+ZLB_CODLAB
		dbSeek(xFILIAL("ZL9")+cpTpFaixa)
	else
		dbSetOrder(1)//ZL9_FILIAL+ZL9_COD
		dbSeek(xFILIAL("ZL9")+cpTpFaixa)
	EndIf
	xnQtdana := ZL9->ZL9_QTDANA
	cpMedia := ZL9->ZL9_MEDIA
	nTotal  := Iif(cpMedia=="G",nTotal:=1,nTotal:=0)//Verifica se eh media aritimetica ou geometrica
	_nMesesRet := ZL9->ZL9_MESRET //Quantidade de meses a retroagir
	nMesAnt := Val(SubStr(DtoS(dpData),5,2))- _nMesesRet

	//??????????????????????????????????????????????????????????
	//? Primeiramente obtem as analises do mes, caso nao tenha ?
	//? busca dos _nMesesRet ultimos meses.                             ?
	//??????????????????????????????????????????????????????????
	...
	While GFZ->(!Eof()) .And. nQtd <= xnQtdana
		...
		If GFZ->ZLB_DESQUA <> "S"
			...
			Do Case
				...
				Case nQtd == 2 //ANALISE 2
					nAna_2 := GFZ->TOTAL
				Case nQtd == 3 //ANALISE 3
					nAna_3 := GFZ->TOTAL
				Case nQtd == 4 //ANALISE 4
					nAna_4 := GFZ->TOTAL
				Case nQtd == 5 //ANALISE 5
					nAna_5 := GFZ->TOTAL
				Case nQtd == 6 //ANALISE 6
					nAna_6 := GFZ->TOTAL
			EndCase

			If cpMedia == "G" //MษDIA GEOMษTRICA
				nTotal*=GFZ->TOTAL
			Else // MษDIA ARITMETICA
				nTotal+=GFZ->TOTAL
			EndIf
		ENDIF
		GFZ->(dbSkip())
	...
	//?????????????????????????????????????????????????????????????????????????????????????????????????
	If _nMesesRet > 0 .and. nQtd == 0 .And. !lDesq
...
User Function PagtoQual(cpTipo,cpCodigo)
	...
	If dbSeek(xFILIAL("ZL9")+cpCodigo)
		...
		nVlrInd := u_getIndice(xFILIAL("ZL8"),ZL2->ZL2_COD,SA2->A2_COD,SA2->A2_LOJA,ZL9->ZL9_CODLAB,ZL9->ZL9_MEDIA,ZLE->ZLE_DTFIM,"3")
		...
		If nVlrInd <> 0
			...
			If cpTipo == "C" // credito
				If nVlrFx > 0
					nTotal += nVlrFx * _pnVLEITE
				EndIf
			ElseIf  cpTipo == "D"// debido
				If nVlrFx < 0
					nTotal += nVlrFx * _pnVLEITE
				EndIf
			ElseIf  cpTipo == "R" // retorna resultado em centavos
				nTotal := nVlrFx
			EndIf
		EndIf
	EndIf
...
ฑฑฬออออออออออุออออออออออสอออออออฯอออออออออออออออออออออออสออออออออออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบDescricao ณ Retorna volume para o evento de fidelidade                                                                  บฑฑ
ฑฑบ          ณ                   												                               									บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                                     						                    		บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
User Function PagtoFidel(xcMesIni,xcMesFim,xcTpRet)

	Local _cCodProd 	:= SA2->A2_COD
	Local _cLoja 	:= SA2->A2_LOJA
	...
	DEFAULT xcMesIni := "01"
	DEFAULT xcMesFim := SubStr(DTOS(dDataBase),5,2)
	DEFAULT xcTpRet := "V" //retorna volume se for T retorna o texto para hist๓rico do evento.

	_cDtIni := _cAno+xcMesIni+"01"
	_cDtFim := _cAno+xcMesFim+"31"


	BeginSql alias _cAlias
		SELECT ZLD_RETIRO AS PROD, SUBSTR(ZLD_DTCOLE,1,6) AS MES, SUM(ZLD_QTDBOM) AS QTDLEITE
		FROM %Table:ZLD%  LD
		WHERE ZLD_RETIRO = %Exp:_cCodProd%
		AND ZLD_RETILJ = %Exp:_cLoja%
		AND ZLD_DTCOLE BETWEEN %Exp:_cDtIni% AND  %Exp:_cDtFim%
		AND LD.%NotDel%
		GROUP BY ZLD_RETIRO , SUBSTR(ZLD_DTCOLE,1,6)
		ORDER BY 1,2 DESC
	EndSql
	(_cAlias)->(DbGoTop())

	If !(_cAlias)->(Eof())
		_nMes := Val(Substr((_cAlias)->MES,5,2))
		_nRet := (_cAlias)->QTDLEITE
		_cUltMes := Substr((_cAlias)->MES,5,2)
		_nMes -= 1
		(_cAlias)->(DbSkip())
	EndIf

	WHile !(_cAlias)->(Eof())
		If  _nMes == Val(Substr((_cAlias)->MES,5,2))
			_nRet += (_cAlias)->QTDLEITE
			_cPriMes := Substr((_cAlias)->MES,5,2)
		EndIf
		_nMes -= 1
		(_cAlias)->(DbSkip())
	EndDo

	_cPriMes := IIF(Empty(_cPriMes),_cUltMes,_cPriMes)

	If xcTpRet == "V"
		Return _nRet
	Else
		_cRet := "Periodo "+_cPriMes+"/"+_cAno+" a "+_cUltMes+"/"+_cAno+": "+Alltrim(Transform(_nRet,"@E 99,999,999"))+"L x "+Alltrim(Transform(ZL8->ZL8_VALOR,"@E 9.999"))
		Return _cRet
	EndIf
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษอออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
...
User Function RGLTCNV(cProdutor,cLoja,cMes,cAno,cDiaPgto,cGrpConv,cMix,lAgluSld)
	...
	Local cEVEESTO	:= GetMv("LT_EVEESTO")
	Local cCONVINT	:= GetMv("LT_CONVINT")
	Local cPrefVen   := POSICIONE("ZL8",1,xFILIAL("ZL8")+Rtrim(cCONVINT),"ZL8_PREFIX")
	...
	ZL8->(dbSetOrder(1))
	If ZL8->(dbSeek(xFilial("ZL8")+cCONVINT))
	...
	If cMixStatus == 'F' // FECHADO
		...
		While !CSQL->(Eof())
			...
			Else
				While !CSQL->(Eof()) .And. CSQL->ZLF_SE1VEN < cDtIni
					If CSQL->ZLF_EVENTO <> ALLTRIM(cEVEESTO)//Codigo do evento de ESTORNO DE LANCAMENTO INDEVIDO - 000028
			...
			If CSQL->ZLF_EVENTO==ALLTRIM(cCONVINT)//"000022"//Convenios Internos
			...
			ElseIf CSQL->ZLF_EVENTO <> ALLTRIM(cEVEESTO)//Codigo do evento de ESTORNO DE LANCAMENTO INDEVIDO - 000028
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function VlrRepFrt(cTanque,cTipo)
	...
	If dbSeek(xFILIAL("ZLS")+cTanque)
		...
		If dbSeek(xFILIAL("ZLT")+cTanque+SA2->A2_COD+SA2->A2_LOJA)
			...
			//Se o evento esta sendo executado para o titular do tanque, retorno o valor total do cr้dito a receber.
			If ZLT->ZLT_TITULA == "S" .AND. cTipo == "C" //ZLT->ZLT_LEITEI == "S"
				//Tipo de Taxa de Frete igual a LITRO.
				If ZLS->ZLS_TPFRT == "L"
					nRet := ZLS->ZLS_TXFRT * U_VLTETQ(cTanque,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM,.T.,.F.)
				ElseIf ZLS->ZLS_TPFRT == "K"//Tipo de Taxa de FRETE igual a KM
					nRet := U_KMTQ(ZLS->ZLS_TXFRT,ZLS->ZLS_KM,(ZLE->ZLE_DTFIM-ZLE->ZLE_DTINI)+1)
				EndIf
			ElseIf ZLT->ZLT_TITULA != "S" .AND. ZLT->ZLT_DESCFR == "S" .AND. cTipo == "D"//O evento esta sendo executado para um participante do tanque, retorna o valor a descontar do produtor
				//Tipo de Taxa de Frete igual a Litro
				If ZLS->ZLS_TPFRT == "L"
					nRet := ZLS->ZLS_TXFRT * _PNVLEITE
				ElseIf ZLS->ZLS_TPFRT == "K"//Tipo de Taxa de FRETE igual a KM
					nVlr := U_KMTQ(ZLS->ZLS_TXFRT,ZLS->ZLS_KM,(ZLE->ZLE_DTFIM-ZLE->ZLE_DTINI)+1)
					nRet := nVlr * (U_PercAgr(cTanque,cTipo,SA2->A2_COD,SA2->A2_LOJA,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM)/100)
					nRet := U_Arred(cTanque,cTipo,SA2->A2_COD,SA2->A2_LOJA,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM,nRet,nVlr,cEvetRep)
				EndIf
			EndIf
		EndIf
	EndIf
		...
		If dbSeek(xFILIAL("ZLT")+cTanque+SA2->A2_COD+SA2->A2_LOJA)
			...
			If ZLT->ZLT_TITULA == "S"
				...
				ElseIf ZLS->ZLS_TPRES == "V" //Tipo de Taxa de Resfriamento igual a Volume
					nRet := U_GETFAIXA(XFILIAL("ZL9"),"000001",cCodFX,U_VLTETQ(cTanque,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM))
					nVlr := nRet
				EndIf
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function VLTETQ(cCodTq,dDtIni,dDtFim,lLeiteiro,lTitular,cTpRet)
	...
	Default lLeiteiro := .F.
	Default lTitular  := .F.
	Default cTpRet := "V" //tipo de retorno, V=Volume, Q = quantidade de produtoers

	cQry := "SELECT SUM(ZLD.ZLD_QTDBOM) AS QTDE, COUNT(ZLT_SA2COD) AS QTDPROD FROM "
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function NroAgreg(cTanque,cTipo,xDtINi,xDtFim)

	Local cArea := GetArea()
	Local cQry  := ""
	Local nTot  := 0
	Local _cDtBetw := "% '"+DTOS(xDtINi)+"' AND '"+DTOS(xDtFim)+"' %"
	Local _cAlias := ""

	If cTipo == "TX"
		_cAlias := GetNextAlias()
		BeginSql alias _cAlias
			SELECT ZLT_SA2COD, ZLT_SA2LJ, SUM(ZLD_QTDBOM)
			FROM %Table:ZLT% ZLT
				JOIN %Table:ZLD% ZLD ON ZLD.ZLD_RETIRO = ZLT.ZLT_SA2COD AND ZLD.ZLD_RETILJ = ZLT.ZLT_SA2LJ
				AND ZLD.ZLD_DTCOLE BETWEEN %Exp:_cDtBetw% AND ZLD.%NotDel%
			WHERE ZLT_FILIAL = %xFILIAL:ZLT%
			AND ZLT_COD    = %Exp:cTanque%
			AND ZLT_DESCRE = 'S'
			AND ZLT.%NotDel%
			GROUP BY ZLT_SA2COD, ZLT_SA2LJ
...
User Function NroDay()
	Local _dDtIni := FirstDay(dDataBase)
	Local _dDtFim := LastDay(dDataBase)

	If Alltrim(FunName()) == "AGLT008" .OR. Alltrim(FunName()) == "AGLT020"
		_dDtIni := ZLE->ZLE_DTINI
		_dDtFim := ZLE->ZLE_DTFIM
	EndIf

Return( (_dDtFim - _dDtIni)+1 )
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัอออออออออออออออออออออออหออออออออออออออออออัออออออออออออออออออออออออออออออออออออออออออออออออปฑฑ
ฑฑบPrograma  ณADCLASSE บ Autor ณ Guilherme Fran็a      บ Data da Criacao  ณ 12/02/2021                						บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯอออออออออออออออออออออออสออออออออออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบDescricao ณ Calcula diferen็a do ZL3_PRECO menos os eventos passados por parโmetro e coloca a diferen็a                  บฑฑ
ฑฑบ          ณ                              							                               						บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ  COOPEAVI evento ADICIONAL DE CLASSE		                                                    											บฑฑ
ฑฑบ          ณ 	                                                        บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบParametrosณ                                                                                                              บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
User Function ADCLASSE(xcEventos)

	Local _nRet := 0
	Local _aEventos := StrToKarr(ALLTRIM(xcEventos),"/")
	Local ix := 0

	For ix := 1 to Len(_aEventos)
		_nRet += U_GetTotEv(xFilial("ZLF"),_aEventos[ix],ZL2->ZL2_COD,ZL3->ZL3_COD,SA2->A2_COD,SA2->A2_LOJA,ZLE->ZLE_COD,"1")
	Next ix

	_nRet := ZL3->ZL3_PRECO - (_nRet/_pnVLEITE)
...
User Function GetResf(cDebCred)
	Local cArea   := GetArea()
	Local nRet    := 0
	Local cTanque := SA2->A2_L_TANQ // U_GetTqPrd(SA2->A2_COD, SA2->A2_LOJA)
	...
	If dbSeek(xFILIAL("ZLS")+cTanque)
		...
		If dbSeek(xFILIAL("ZLT")+cTanque+SA2->A2_COD+SA2->A2_LOJA)
			...
			If ZLT->ZLT_TITULA == "S" .and. cDebCred == "C"
				//Tipo de Taxa de Resfriamento igual a valor Fixo.
				If ZLS->ZLS_TPRES == "F"
					nRet := ZLS->ZLS_TXRES *  U_NroAgreg(cTanque,'TX',ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM) //U_VLTETQ(cTanque,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM,.f.,.t.,"Q")
				ElseIf ZLS->ZLS_TPRES == "V" //Tipo de Taxa de Resfriamento igual a Volume
					nRet := ZLS->ZLS_TXRES
					nRet := nRet * U_VLTETQ(cTanque,ZLE->ZLE_DTINI,ZLE->ZLE_DTFIM,.f.,.t.,"V")
				EndIf
			ElseIf ZLT->ZLT_TITULA != "S" .AND. ZLT->ZLT_DESCRE == "S" .and. cDebCred == "D"//O evento esta sendo executado para um agregado
				//Tipo de Taxa de Resfriamento igual a valor Fixo
				If ZLS->ZLS_TPRES == "F"
					nRet := ZLS->ZLS_TXRES
				ElseIf ZLS->ZLS_TPRES == "V" //Tipo de Taxa de Resfriamento igual a Volume
...
User Function Funrural(xValor)
	...
	Local _lEmbFun := SuperGetMv("LT_EMBFUNR",,.F.) //Indica se a empresa embute o valor do funrural no pre็o, para o produtor nใo ter esta despesa.
	Local _nFator	:= 0
	Local _nRet := 0
	Local _nAliqFun := (ZL2->ZL2_ALQINS + ZL2->ZL2_ALQSEN + ZL2->ZL2_ALQRAT) / 100 // GetMv("LT_ALIQFUN") / 100
	Local _nAliqSen := ZL2->ZL2_ALQSEN / 100 //GetMv("LT_ALIQSEN") / 100
	Local _nAliquota := 0
	Local _cAlias := GetNextAlias()
	Local _aArea := GetArea()
	DEFAULT xValor := 0
	...
	//O produtor sempre tem o cแlculo completo do funrural, ou caso tenha liminar, terแ somente o cแlculo do SENAR
	If SA2->A2_L_INSS == "S"  .AND. SA2->A2_L_SENAR == "S"
		_nFator := 1 - _nAliqFun  // 1,5%
		_nAliquota := _nAliqFun
	ElseIf SA2->A2_L_SENAR == "S"
		_nFator := 1 - _nAliqSen // 0,2%
		_nAliquota := _nAliqSen
	EndIf
	...
	If _lEmbFun
		If _nFator > 0
			...
			_nRet := _nRet - xValor // Retorna o valor do funrural embutindo no valor principal
		EndIf
	Else
		_nRet := xValor * _nAliquota // Retorna conta simples: Valor X Aliquota (1000 * 1,5%)
	EndIf
...
/*
===============================================================================================================================
Programa----------: DEVFUNRURAL
Autor-------------: GUILHERME FRANวA
Data da Criacao---: 16/03/2021
===============================================================================================================================
...
User Function DEVFUNRURAL()
	...
	BeginSql alias "TNFL"
		...
		FROM %Table:SF1%  F1
		WHERE  F1.F1_FILIAL = %xFilial:SF1%
		AND F1.F1_FORNECE = %Exp:SA2->A2_COD%
		AND F1.F1_LOJA = %Exp:SA2->A2_LOJA%
		AND F1.F1_L_MIX = %Exp:ZLE->ZLE_COD%
		AND F1.%NotDel%
...
Data da Criacao---: 16/03/2021
===============================================================================================================================
Descricao---------: Retorna valor do cr้dito da associa็ใo

===============================================================================================================================
Uso---------------: Modulo Gestao do Leite
===============================================================================================================================
*/
User Function CREDASSOC(xcCod,xcMix,xcTpRet)
	...
	dbSelectArea("ZLE")
	dbSetOrder(1)
	MsSeek(xFILIAL("ZLE")+xcMix)
	_cFiltro := "%  "+DTOS(ZLE->ZLE_DTINI)+" AND "+DTOS(ZLE->ZLE_DTFIM)+"  %"

	BeginSql alias _cAlias
		SELECT  ZLD_RETIRO, SUM(ZLD_QTDBOM) AS QTDBOM
		FROM %Table:ZLD% ZLD
			JOIN %Table:SA2% A2 ON A2.A2_COD = ZLD.ZLD_RETIRO AND A2.A2_LOJA = ZLD.ZLD_RETILJ AND A2.A2_L_CODAS = %Exp:xcCod% and A2.%NotDel%
	...
	DbSelectArea("ZM3")
	ZM3->(DbSetOrder(1))
	If ZM3->(MsSeek(xFilial("ZM3")+xcCod))
		If ZM3->ZM3_DESCLT > 0 //Cr้dito por litragem da associa็ใo
			_nRet := _nVolume * ZM3->ZM3_DESCLT
			_cRet := Alltrim(cValToChar(_nVolume))+" litros X R$"+Transform(ZM3->ZM3_DESCLT,"@E 99.9999")
		ElseIf ZM3->ZM3_DESCVL > 0 //Cr้dito por litragem da associa็ใo
			_nRet := _nQtdProd * ZM3->ZM3_DESCVL
			_cRet := Alltrim(cValToChar(_nQtdProd))+" associados X R$"+Transform(ZM3->ZM3_DESCVL,"@E 9,999.99")
		EndIf
	EndIf
